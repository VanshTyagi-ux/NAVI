...
function renderSubjectOverview() {
    ...
    // Inside teacher.js -> showContentDetails function
        ...
        <div class="space-y-2">
            ${videosForChapter.map(video => `
                <div class="bg-gray-50 dark:bg-gray-700/50 p-2.5 rounded-lg flex items-center justify-between">
                    ...
                    <button onclick="import('./offlineSync.js').then(m => m.sendFileThroughChannel('${video.fileUrl}', '${video.fileName}', 'video/mp4'))" 
                            class="text-xs bg-orange-500 text-white font-semibold py-1 px-3 rounded-md">Sync 📶</button>
                </div>
            `).join('')}
            ${notesForChapter.map(note => `
                <div class="bg-gray-50 dark:bg-gray-700/50 p-2.5 rounded-lg flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="openNotesViewer('${note.fileUrl}', '${note.fileName}')" class="text-xs bg-blue-500 text-white font-semibold py-1 px-3 rounded-md">View</button>
                        <button onclick="import('./offlineSync.js').then(m => m.sendFileThroughChannel('${note.fileUrl}', '${note.fileName}', 'application/pdf'))" 
                                class="text-xs bg-orange-500 text-white font-semibold py-1 px-3 rounded-md">Sync 📶</button>
                    </div>
                </div>
            `).join('')}
            ${assignmentsForChapter.map(assignment => `
                <div class="bg-gray-50 dark:bg-gray-700/50 p-2.5 rounded-lg flex items-center justify-between">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="openNotesViewer('${assignment.fileUrl}', '${assignment.fileName}')" class="text-xs bg-amber-500 text-white font-semibold py-1 px-3 rounded-md">View</button>
                        <button onclick="import('./offlineSync.js').then(m => m.sendFileThroughChannel('${assignment.fileUrl}', '${assignment.fileName}', 'application/pdf'))" 
                                class="text-xs bg-orange-500 text-white font-semibold py-1 px-3 rounded-md">Sync 📶</button>
                    </div>
                </div>
            `).join('')}
