 ...
 */
// Add to offlineSync.js

/**
 * Sends a file (from an object URL or Blob) over the data channel
 */
export async function sendFileThroughChannel(fileData, fileName, fileType) {
    if (!dataChannel || dataChannel.readyState !== 'open') {
        alert("Not connected to a student!");
        return;
    }
...
// Update setupDataChannelEvents to handle receiving chunks
let receivedChunks = [];
...
function setupDataChannelEvents() {
    ...
    // ... existing onopen and onclose logic
}

function finalizeFileDownload() {
    const blob = new Blob(receivedChunks, { type: incomingFileInfo.fileType });
    const url = URL.createObjectURL(blob);
    
    // Trigger download or save to local storage
